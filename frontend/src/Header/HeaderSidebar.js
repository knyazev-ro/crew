...
export default function HeaderSidebar({ onHide, collapsed }) {
  ...
  return (
    <div className="">
      <div className="transition-all duration-300 ease-in-out items-center gap-2 py-2 h-12 justify-center flex">
        <div
          className={classNames(
            "flex bg-stone-200 w-3/4 h-7 rounded-full py-1 px-3 items-center",
            {
        ...
        <button
          className={classNames(
            "text-rich_black rounded-full w-12 h-7 flex border border-purple_sky-100 items-center justify-center hover:text-purple_sky-100",
            {
              "-translate-x-5": collapsed,
            }
          )}
          onClick={onHide}
        >
